...
/// here the state is created i.e. the state of the map is created.
/// the map state comprises cleaned and uncleaned vectors that store points
/// that represent the state of the map.
    ...
    for line in reader.lines() {
        ...
        if store.contains("X") {
            for c in store.chars() {
                match c {
                    'S' => {
                        state.start = Some(Point::new(m, n));
                    ...
                    'P' => {
                        state.portals.push(Point::new(m, n));
                    }
                    _ => (),
                }
                n = n + 1usize;
...
/// This is a helper method to process the state.
/// this method checks for the state if a given plan is valid.
...
/// This is also a helper method that combines the previous functionalities and
/// processes the state if the start is not given.
pub fn process_state_start_not_given(st: &mut State) -> HashSet<Point> {
    ...
    for i in unclean {
        ...
        state
            .uncleaned
            .remove(state.uncleaned.binary_search(&i).unwrap());
...
/// This function create the respective files and writes the answers to the file.
pub fn write_to_file_start_given(
    state: State,
    path: &mut PathBuf,
    filename: &str,
) -> std::io::Result<()> {
...
pub fn write_to_file_start_not_given(
    set: HashSet<Point>,
    path: &mut PathBuf,
    filename: &str,
) -> std::io::Result<()> {
...
pub fn write_to_file_start_not_given_find(
    state: &mut State,
    path: &mut PathBuf,
    filename: &str,
) -> std::io::Result<()> {
...
/// this function takes an entire directory and gives y solution file within the directory.
            ...
            if p.is_file() {
                ...
                if state.start != None {
                    ...
                    if state.check {
                        ...
                        write_to_file_start_not_given_find(&mut state, &mut path.clone(), file_name)
                            .unwrap()
                    }
                }
            }
